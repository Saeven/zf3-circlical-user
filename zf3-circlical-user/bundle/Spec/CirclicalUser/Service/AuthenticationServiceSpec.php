...
class AuthenticationServiceSpec extends ObjectBehavior
{
    ...
    public function let(AuthenticationMapper $authenticationMapper, UserMapper $userMapper)
    {
    ...
    public function it_permits_username_changes($authenticationMapper, $userMapper)
    {
        $this->changeUsername($this->userData, 'newUsername');
    }

    public function it_does_nothing_for_same_username_changes($authenticationMapper, $userMapper)
    {
        $this->changeUsername($this->userData, 'userA');
    }

    public function it_declines_username_changes_for_missing_users($authenticationMapper, $userMapper)
    {
        $user = new User();
        $user->setId(3);
        $this->shouldThrow(NoSuchUserException::class)->during('changeUsername', [$user, 'newUsername']);
    }

    public function it_declines_username_changes_when_usernames_are_taken($authenticationMapper, $userMapper)
    {
        $user = new User();
        $user->setId(1);
        $this->shouldThrow(UsernameTakenException::class)->during('changeUsername', [$user, 'orphan']);
    }
    ...
    public function it_authenticates_with_cookies()
    {
        ...
        $hashCookieContents = base64_encode(Crypto::encrypt(time() . ':' . $this->authenticationData->getUserId() . ':' . $this->authenticationData->getUsername(), $userKey));

        $_COOKIE[AuthenticationService::COOKIE_USER] = $userTuple;
        $_COOKIE[$hashCookieName] = $hashCookieContents;
    ...
    public function it_bails_when_the_user_tuple_is_not_well_formatted()
    {
        ...
        $userKey = new EncryptionKey($this->authenticationData->getSessionKey());
        $hashCookieName = hash_hmac('sha256', $this->authenticationData->getSessionKey() . $this->authenticationData->getUsername(), $systemKey);
        $userTuple = base64_encode(Crypto::encrypt('tanqueray', $systemKey));
        $hashCookieContents = base64_encode(Crypto::encrypt(time() . ':' . $this->authenticationData->getUserId() . ':' . $this->authenticationData->getUsername(), $userKey));
    ...
    public function it_bails_when_the_user_tuple_contains_well_formatted_garbage()
    {
        ...
        $userKey = new EncryptionKey($this->authenticationData->getSessionKey());
        $hashCookieName = hash_hmac('sha256', $this->authenticationData->getSessionKey() . $this->authenticationData->getUsername(), $systemKey);
        $userTuple = base64_encode(Crypto::encrypt('A' . ":" . $hashCookieName, $systemKey));
        $hashCookieContents = base64_encode(Crypto::encrypt(time() . ':' . $this->authenticationData->getUserId() . ':' . $this->authenticationData->getUsername(), $userKey));
    ...
    public function it_bails_when_the_user_tuple_contains_an_impossible_user_id()
    {
        ...
        $userKey = new EncryptionKey($this->authenticationData->getSessionKey());
        $hashCookieName = hash_hmac('sha256', $this->authenticationData->getSessionKey() . $this->authenticationData->getUsername(), $systemKey);
        $userTuple = base64_encode(Crypto::encrypt(15234 . ":" . $hashCookieName, $systemKey));
        $hashCookieContents = base64_encode(Crypto::encrypt(time() . ':' . $this->authenticationData->getUserId() . ':' . $this->authenticationData->getUsername(), $userKey));
    ...
    public function it_bails_when_the_hash_cookie_is_not_well_formatted()
    {
        ...
        $hashCookieName = hash_hmac('sha256', $this->authenticationData->getSessionKey() . $this->authenticationData->getUsername(), $systemKey);
        $userTuple = base64_encode(Crypto::encrypt($this->authenticationData->getUserId() . ":" . $hashCookieName, $systemKey));
        $hashCookieContents = base64_encode(Crypto::encrypt($this->authenticationData->getUserId() . ':' . $this->authenticationData->getUsername(), $userKey));
    ...
    public function it_fails_when_any_cookies_are_missing()
    {
        ...
        $cookies = [
            ...
            $hashCookieName => $hashCookieContents,
    ...
    public function it_resets_passwords($authenticationMapper)
    {
        $authenticationMapper->findByUserId(1)->shouldBeCalled();
        $authenticationMapper->update($this->authenticationData)->shouldBeCalled();
        $this->resetPassword($this->userData, 'efg');
    }

    public function it_wont_reset_passwords_when_users_do_not_exist()
    {
        $user = new User();
        $user->setId(5);
        $this->shouldThrow(NoSuchUserException::class)->during('resetPassword', [$user, 'efg']);
    }
    ...
    public function it_can_create_new_auth_records($authenticationMapper)
    {
        ...
        $this->create($user, 'userC', 'abcd')->shouldBeAnInstanceOf(Authentication::class);
    }

    public function it_wont_overwrite_existing_auth_on_create($authenticationMapper)
    {
        $user = new User();
        $user->setId(5);
        $this->shouldThrow(UsernameTakenException::class)->during('create', [$user, 'userA', 'abcd']);
    }

    public function it_wont_create_auth_when_email_usernames_belong_to_user_records($authenticationMapper)
    {
        $user = new User();
        $user->setId(5);
        $user->setEmail('alex@circlical.com');
        $this->shouldThrow(EmailUsernameTakenException::class)->during('create', [$user, 'alex@circlical.com', 'abcd']);
    }

    public function it_does_not_permit_mismatched_emails()
    {
        $user = new User();
        $user->setEmail('a@b.com');
        $this->shouldThrow(MismatchedEmailsException::class)->during('create', [$user, 'b@b.com', 'abcd']);
    }
